...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  if (cmd === 'start' && positional[1]) {
    return { command: 'start', goal: positional.slice(1).join(' '), maxSteps, verbose };
  }
...
async function main() {
  ...
  switch (args.command) {
    case 'repl': {
      ...
      await replCommand(args.maxSteps, args.verbose);
      break;
    }
    case 'start': {
      const { startCommand } = await import('./start.js');
      await startCommand(args.goal!, args.maxSteps, args.verbose);
      break;
    }
    case 'config': {
  ...
  harness start --verbose        Show step-by-step agent output
  ...
  harness config set-model       Set model name (default: gpt-4o-mini)
  harness config view-model      View current model name
  harness config clear-model     Clear model name (use default)
